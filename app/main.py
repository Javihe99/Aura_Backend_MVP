...
async def chat_search(request: ChatRequest, background_tasks: BackgroundTasks):
    ...
    try:
        # OPTIMIZACIÓN 1: Procesamiento paralelo de tareas independientes
        memory_manager = get_service(app, 'memory_manager')

        # Ejecutar tareas en paralelo
        history_task = memory_manager.get_conversation_history(session_id)

        # Guardar mensaje del usuario en background (no bloquea)
        ...
        # OPTIMIZACIÓN 2: Clasificación de intención más rápida
        ...
        # OPTIMIZACIÓN 3: Procesamiento paralelo de LLM y validación de ubicación
        ...
        # OPTIMIZACIÓN CRÍTICA: Procesamiento ultra-paralelo
        loop = asyncio.get_event_loop()

        # 1. LLM y validación de ubicación en paralelo (más agresivo)
        llm_task = loop.run_in_executor(None, get_llm_result, enhanced_prompt)

        # Extraer ubicación del prompt directamente para validación paralela
        ...
        # OPTIMIZACIÓN CRÍTICA: Coordenadas y búsqueda en paralelo
        coordinates_task = loop.run_in_executor(
            None, get_area_by_giving_location, best_location
        )

        # Preparar prompt_result optimizado mientras obtenemos coordenadas
        prompt_result_copy = prompt_result.copy()
        prompt_result_copy['limit'] = min(request.limit * 2, 50)  # Límite optimizado pero respetando request

        # Esperar coordenadas y crear polígono
        coordinates = await coordinates_task
        geojson_str = to_idealista_multipolygon(coordinates)
        prompt_result_copy['shape'] = geojson_str

        # OPTIMIZACIÓN CRÍTICA: Búsqueda de propiedades con límites agresivos
        ...
        if df_raw.empty:
            ...
            # Guardar mensaje del asistente en background
            background_tasks.add_task(
                memory_manager.save_message,
        ...
        # OPTIMIZACIÓN CRÍTICA: Filtros ultra-rápidos y respuesta temprana
        quality_filter = get_service(app, 'quality_filter')

        # OPTIMIZACIÓN AGRESIVA: Filtros mínimos para respuesta rápida
        ...
        # OPTIMIZACIÓN CRÍTICA: Resumen con LLM optimizado
        summarizer = get_service(app, 'summarizer')

        if records:
            # Generar resumen con LLM pero optimizado
            summary_context = f"Consulta: {request.prompt}"  # Contexto simplificado
        ...
        # OPTIMIZACIÓN CRÍTICA: Preparar datos de forma ultra-eficiente
        prompt_result_clean = prompt_result.copy()
        prompt_result_clean.pop('shape', None)

        # Extraer property_list de forma eficiente
        property_list = df['propertyCode'].tolist() if not df.empty else []

        # OPTIMIZACIÓN CRÍTICA: Background tasks mínimos
        property_manager = get_service(app, 'property_manager')
        properties_data = df_raw.head(request.limit).to_dict('records') if not df_raw.empty else []  # Respetar límite del request
        ...
        # OPTIMIZACIÓN CRÍTICA: Background tasks no bloquean respuesta
        background_tasks.add_task(
...
async def maps_search(request: MapSearchRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        df = quality_filter.filter_and_rank_properties(
            df_raw,
            top_n=request.limit,  # Respetar el límite del request
            paraphrase_descriptions=False,
            max_concurrent=5
        )
...
async def create_appointment(request: AppointmentRequest, http_request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Manejar caso especial de TestClient
